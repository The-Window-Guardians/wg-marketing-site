...
: '';
    ...
    const ANGLE_CREDIT =
'- NO INVENTED BACKSTORY: only describe THIS home’s OLD/before condition (drafts, fog, rot, "failing for years") if a BEFORE photo is attached OR the notes state it. With only the finished result, do NOT narrate this specific house’s past as if you saw it. You MAY still use the old-window villain in a GENERAL or hypothetical way ("old windows love to fog up and quit" / "if your windows are foggy…"), or simply celebrate the new result — just never assert a before-state for THIS job that you cannot see.\n' +
